...
def plot_compare(levels, track, aud, d, d_ci, perm_p, rho, rho_p, kinds):
    ...
    # (2) THE HYPOTHESIS: distraction effect (d) vs hierarchy level + trend
    ...
    b, a0 = np.polyfit(lev, d, 1)
    ax.plot(lev, a0 + b * lev, "--", color="#555", lw=1.5, label="linear trend")
    ax.axhline(0, color="k", lw=.7)
    ax.set_xticks(lev); ax.set_xticklabels([PRETTY[l].replace(" ", "\n") for l in levels], fontsize=9)
    ax.set_ylabel("distraction effect  (Cohen's d, focused − distracted)")
    ax.set_title("Does visual distraction hit HIGHER speech levels more?\n"
                 f"Spearman ρ = {rho:+.2f} (p = {rho_p:.3f})",
                 fontsize=11)
...
def main():
    ...
    for l in levels:
        track[l] = feature_tracking(EEGr, [f.astype(np.float64) for f in FEATS[l]], "cca2", kcc)
    ...
    out = {"created": _dt.datetime.now().isoformat(timespec="seconds"), "subject": "sub001_Kavin",
           ...
           "levels": levels, "kinds": kinds, "method": "aud_cca regularised CCA (cca2), top-%d canonical r" % kcc,
