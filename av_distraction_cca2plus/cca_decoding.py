...
Figures (av_distraction/figures/):
  ...
  cca_best.png       best test score per model {backward, forward, CCA1, CCA2, CCA2+},
...
MODELS5 = ["backward", "forward", "cca1", "cca2", "cca2plus"]
...
def main():
    ...
    for f in FEATURES:
        for c, idx in conds.items():
            ...
            cc = {m: cca_curve(m, E, Fl, shift_samp, folds, KTHIN) for m in ("cca1", "cca2", "cca2plus")}
...
def fig_best(R):
    ...
    for i, f in enumerate(FEATURES):
        ...
        a.set_xticks(x); a.set_xticklabels(["backward", "forward", "CCA1", "CCA2", "CCA2+"], fontsize=9)
...
def fig_decoding_topo(R, shifts, EEG, FEATS, aud, info, fs):
    """Clean per-feature decoding (per-trial CCA2+ canonical correlation, Audio vs Visual) + topography."""
    fig, ax = plt.subplots(len(FEATURES), 2, figsize=(11, 3.0 * len(FEATURES)),
                           gridspec_kw={"width_ratios": [1.4, 1]})
    for i, f in enumerate(FEATURES):
        # per-trial CCA2+ top-canonical-r (LOO-ish: fit others, score trial). CCA2+'s lag basis
        # models the response timing, so no external shift is applied (L = 0).
        pt = {}
        for c, idx in (("Audio", np.where(aud)[0]), ("Visual", np.where(~aud)[0])):
            E = [EEG[j] for j in idx]; Fl = [FEATS[f][j] for j in idx]
            sc = np.zeros(len(E))
            for a, b in _folds(len(E), 5):
                m = M.model("cca2plus").fit([E[j] for j in a], [Fl[j] for j in a])
                for j in b:
    ...
    fig.suptitle("Per-trial CCA2+ decoding by feature (focused vs distracted) + topography", fontsize=13)
